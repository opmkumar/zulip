...
export function set_up_attachments(): void {
    ...
    $("#attachments-settings").on("click", ".clear_filter", (e) => {
        e.stopPropagation();
        e.preventDefault();

        const $container = $("#attachments-settings").find(".search_container");
        const $filter = $container.find(".search");
        uploaded_files_list_widget.clear_text_filter($filter);
    });
