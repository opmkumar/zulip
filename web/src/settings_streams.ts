...
export function build_page(): void {
    ...
    $("#admin-default-channels-list").on("click", ".clear_filter", (e) => {
        e.stopPropagation();
        e.preventDefault();

        const $container = $("#admin-default-channels-list").find(".search_container");
        const $filter = $container.find(".search");
        default_stream_list_widget.clear_text_filter($filter);
    });
}
