...
let muted_users_list_widget: ListWidgetType<MutedUserItem, MutedUserItem>;
...
export function populate_list(): void {
    ...
    muted_users_list_widget = ListWidget.create<MutedUserItem>(
        $muted_users_table,
        all_muted_users,
        {
...
export function set_up(): void {
    ...
    $("#muted-user-settings").on("click", ".clear_filter", (e) => {
        e.stopPropagation();
        e.preventDefault();

        const $container = $("#muted-user-settings").find(".search_container");
        const $filter = $container.find(".search");
        muted_users_list_widget.clear_text_filter($filter);
    });
